...
def chunk_text(cleaned_file, max_length=300, overlap=100, output_dir="chunks"):
    """
    ...
    for word in words:
        chunk.append(word)
        if len(chunk) >= max_length:   # 800 words per chunk
...
def run_pipeline_for_folder(pdf_folder):
    ...
    for pdf in pdf_files:
        ...
        country = os.path.splitext(os.path.basename(pdf))[0]  # file name without extension
